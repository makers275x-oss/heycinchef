...
function hasMeasure(text: string) {
  return /(\d|ml|cl|g|kg|gr|adet|dilim|bardak|kase|kaşık|tatlı kaşığı|yemek kaşığı|çay kaşığı|demet|dal|yaprak|küp|oz|cup|tbsp|tsp)/i.test(
    String(text || "")
  );
}
...
function prettyNumber(n: number) {
  ...
  return Number.isInteger(fixed) ? String(fixed) : fixed.toFixed(1).replace(".0", "");
}

function recipeFallbackIngredient(name: string, persons: number) {
  const n = normalize(name);

  if (hasMeasure(name)) return name;
  if (n.includes("domates")) return `${Math.max(1, persons)} adet domates`;
  if (n.includes("limon")) return `${Math.max(1, Math.ceil(persons / 2))} adet limon`;
  if (n.includes("mantar")) return `${150 * Math.max(1, persons)} g mantar`;
  if (n.includes("fesleğen")) return `${Math.max(4, persons * 3)} yaprak fesleğen`;
  if (n.includes("maydanoz")) return `${Math.max(2, persons)} yemek kaşığı maydanoz`;
  if (n.includes("zeytinya")) return `${Math.max(1, persons)} yemek kaşığı zeytinyağı`;
  if (n === "tuz" || n.includes("tuz")) return `${prettyNumber(Math.max(0.5, persons * 0.5))} çay kaşığı tuz`;
  if (n.includes("karabiber")) return `${prettyNumber(Math.max(0.25, persons * 0.25))} çay kaşığı karabiber`;
  if (n === "su" || n.includes("su")) return `${Math.max(100, persons * 100)} ml su`;
  if (n.includes("soğan")) return `${Math.max(1, Math.ceil(persons / 2))} adet soğan`;
  if (n.includes("sarımsak")) return `${Math.max(1, persons)} diş sarımsak`;
  if (n.includes("peynir")) return `${50 * Math.max(1, persons)} g ${name}`;
  if (n.includes("makarna") || n.includes("pirinç") || n.includes("bulgur")) return `${75 * Math.max(1, persons)} g ${name}`;

  return `${Math.max(1, Math.ceil(persons / 2))} adet ${name}`;
}

function cocktailFallbackIngredient(name: string, level: AlcoholLevel) {
  const n = normalize(name);
  ...
  const spiritMl = spiritMap[level];
  const mixerMl = mixerMap[level];

  if (n.includes("buz")) return "5-6 küp buz";
  if (n.includes("lime")) return "15 ml lime suyu";
  if (n.includes("limon")) return "15 ml limon suyu";
  if (n.includes("nane")) return "6-8 yaprak nane";
  if (n.includes("şeker şurubu") || n.includes("syrup") || n.includes("şurup")) return "10 ml şurup";
  if (n.includes("tonik") || n.includes("soda") || n.includes("kola") || n.includes("meyve suyu")) {
    return `${mixerMl} ml ${name}`;
  }

  return `${spiritMl} ml ${name}`;
}
...
function ChefCin({
  ...
  return (
    <div className="select-none pointer-events-none">
      ...
      <div className="pointer-events-auto ml-auto w-[170px] sm:w-[205px] genie-float">
        <button
          type="button"
          onClick={onClick}
          onKeyDown={(e) => {
...
export default function FridgeChefApp() {
  ...
  const [showChefWelcome, setShowChefWelcome] = useState(false);
  ...
  const lastCaptureKeyRef = useRef<string>("");
  ...
  useEffect(() => {
    ...
    const t2 = setTimeout(() => {
      unlockTTS();
      void speak("Hoş geldiniz. Cin Şef burada. Hazırsanız büyüyü başlatalım.", true);
    }, 1500);
  ...
  const displayedIngredients = useMemo(() => {
    const raw = screen === "recipe" ? recipe?.ingredients || [] : cocktail?.ingredients || [];

    return raw.map((it) =>
      screen === "recipe"
        ? recipeFallbackIngredient(it, persons)
        : cocktailFallbackIngredient(it, alcoholLevel)
    );
  }, [screen, recipe, cocktail, persons, alcoholLevel]);
  ...
  async function optimizeImageForUpload(file: File, maxWidth = 1280, quality = 0.78): Promise<File> {
  ...
  function localPosterDataUrl(input: {
    mode: "recipe" | "cocktail";
    title: string;
    summary: string;
    ingredients: string[];
  }) {
    const title = truncateText(input.title || (input.mode === "cocktail" ? "Cin Kokteyli" : "Cin Tarifi"), 42);
    const summary = truncateText(input.summary || "", 120);
    const ingredients = (input.ingredients || []).slice(0, 5).map((x) => truncateText(x, 24)).join(" • ");
    const badge = input.mode === "cocktail" ? "AI BAR SUNUMU" : "AI TABAK SUNUMU";
    const emoji = input.mode === "cocktail" ? "🍸" : "🍽️";
    const bg1 = input.mode === "cocktail" ? "#0f172a" : "#1f2937";
    const bg2 = input.mode === "cocktail" ? "#7c3aed" : "#f59e0b";
    const card = input.mode === "cocktail" ? "#ffffff" : "#fffaf0";
    ...
    const svg = `
      <svg xmlns="http://www.w3.org/2000/svg" width="1024" height="1024" viewBox="0 0 1024 1024">
        ...
        <circle cx="860" cy="180" r="130" fill="rgba(255,255,255,0.14)"/>
        <circle cx="180" cy="880" r="180" fill="rgba(255,255,255,0.10)"/>
        <rect x="92" y="92" rx="42" ry="42" width="840" height="840" fill="${card}" filter="url(#shadow)"/>
        <text x="140" y="192" font-size="44" font-weight="700" fill="#6b7280" font-family="Arial, sans-serif">${badge}</text>
        <text x="140" y="286" font-size="92" font-weight="700" fill="#111827" font-family="Arial, sans-serif">${emoji}</text>
        <text x="140" y="380" font-size="58" font-weight="700" fill="#111827" font-family="Arial, sans-serif">${title}</text>
        <foreignObject x="140" y="420" width="744" height="160">
          <div xmlns="http://www.w3.org/1999/xhtml" style="font-family: Arial, sans-serif; font-size: 30px; color:#374151; line-height:1.45;">
            ${summary}
          </div>
        </foreignObject>
        <rect x="140" y="640" rx="26" ry="26" width="744" height="150" fill="#f3f4f6"/>
        <text x="176" y="700" font-size="30" font-weight="700" fill="#111827" font-family="Arial, sans-serif">Malzemeler</text>
        <foreignObject x="176" y="724" width="670" height="50">
          <div xmlns="http://www.w3.org/1999/xhtml" style="font-family: Arial, sans-serif; font-size: 24px; color:#4b5563; line-height:1.4;">
            ${ingredients || "Sunum hazırlanıyor"}
          </div>
        </foreignObject>
        <text x="140" y="860" font-size="28" font-weight="700" fill="#6b7280" font-family="Arial, sans-serif">Cin Şef • AI sunum önizleme</text>
      </svg>
    `;
  ...
  async function generateDishImage(input: {
    ...
    const fallbackUrl = localPosterDataUrl(input);
    ...
    const prompt =
      input.mode === "cocktail"
        ? `Ultra realistic premium cocktail photography, studio lighting, elegant glass, cinematic bar background, appetizing drink styling, condensation, garnish, top tier beverage commercial shot. Drink name: ${input.title}. Summary: ${input.summary}. Ingredients: ${(input.ingredients || []).join(", ")}`
        : `Ultra realistic gourmet food photography, premium plating, soft studio lighting, cinematic restaurant style, detailed textures, appetizing composition, professional food commercial shot. Dish name: ${input.title}. Summary: ${input.summary}. Ingredients: ${(input.ingredients || []).join(", ")}`;

    const candidateRoutes = ["/api/dish-image", "/api/image", "/api/generate-image"];

    try {
      for (const route of candidateRoutes) {
        try {
          const data = await postJson<any>(route, {
            mode: input.mode,
            title: input.title,
          ...
          // sonraki route denenir
        }
      }

      setDishImageUrl(fallbackUrl);
    } catch {
      setDishImageUrl(fallbackUrl);
    } finally {
      setIsImageGenerating(false);
  ...
  async function ensureNativeCameraPermission() {
    ...
    try {
      const perms: any = await Camera.checkPermissions();

      if (perms?.camera === "granted") {
        return true;
      }

      const requested: any = await Camera.requestPermissions();
  ...
  useEffect(() => {
    if (!introDone) return;
    if (screen === "home" && !homeStarted) return;
  ...
  }, [screen, homeStarted, introDone, voices.length, nativeTtsEnabled]);
  ...
  const applyNativePhoto = useCallback(
    async (photo: NativePhotoLike) => {
      setError("");
      setRecipe(null);
      setCocktail(null);
      setDishImageUrl("");
      ...
      safeRevokeObjectUrl(imagePreviewUrl);

      const captureKey =
        photo?.webPath ||
        photo?.dataUrl?.slice(0, 120) ||
        photo?.base64String?.slice(0, 120) ||
        String(Date.now());
      ...
      try {
        if (photo?.dataUrl) {
          ...
          builtFile = dataUrlToFile(photo.dataUrl, `capture-${Date.now()}.jpg`);
          setImagePreviewUrl(previewUrl);
        } else if (photo?.base64String) {
          const mime = `image/${photo.format || "jpeg"}`;
          previewUrl = `data:${mime};base64,${photo.base64String}`;
          builtFile = dataUrlToFile(previewUrl, `capture-${Date.now()}.jpg`);
          setImagePreviewUrl(previewUrl);
        } else if (photo?.webPath) {
          previewUrl = `${photo.webPath}${photo.webPath.includes("?") ? "&" : "?"}t=${Date.now()}`;
          setImagePreviewUrl(previewUrl);

          const resp = await fetch(photo.webPath);
          const blob = await resp.blob();
          builtFile = new File([blob], `capture-${Date.now()}.jpg`, {
            type: blob.type || "image/jpeg",
          });
        }
      } catch {
        setError("Foto okunamadı");
        return;
      }

      if (!previewUrl || !builtFile) {
        setError("Foto alınamadı");
        return;
      }

      setImageFile(builtFile);
      resetIdleTimer();
  ...
  useEffect(() => {
    ...
    App.addListener("appRestoredResult", ({ pluginId, methodName, data, success }) => {
      if (!success) return;
      if (pluginId !== "Camera" || methodName !== "getPhoto") return;
      if (!data) return;

      void applyNativePhoto(data as NativePhotoLike);
    }).then((handle) => {
  ...
  }, [screen, homeStarted, isScanning, isGenerating]);
  ...
  async function pickImageNative(source: "camera" | "gallery" = "camera") {
    try {
      setError("");
      setRecipe(null);
      setCocktail(null);
      ...
      void applyNativePhoto(photo as NativePhotoLike);
    } catch (e: any) {
  ...
  async function scanImage() {
    ...
    if (!imageFile) {
      ...
      void speak("Foto yok… önce seçelim 😄");
      return;
    }
    ...
    void speak(
      type === "food"
        ? "Dolabı tarıyorum… içindekiler saklanmasın 👀"
        : "Etiketleri okuyorum… kaçamazlar 👀"
    );

    try {
      const fastFile = await optimizeImageForUpload(imageFile, 1280, 0.78);

      const fd = new FormData();
      fd.append("image", fastFile);
      fd.append("type", type);
      ...
      if (type === "food") {
        ...
        void speak(
          list.length
            ? `Buldum: ${list.map((x) => x.name).join(", ")} 😎`
            : "Bir şey göremedim… daha net çek 😅"
        );
      } else {
        ...
        void speak(
          list.length
            ? "Etiketleri okudum. Bar açıldı 😎"
            : "Etiket okunmuyor… daha net çek ya da manuel ekle 😄"
        );
      }
      ...
      setError(msg);
      void speak("Tarama biraz dramatik bitti… bir daha deneyelim 😅");
    } finally {
      setIsScanning(false);
    }
  }
  ...
  async function generateRecipe(nextTry?: number) {
    ...
    resetIdleTimer();
    void speak("Tarif yazıyorum… mutfakta olay var 😎");
  ...
  async function generateCocktail(nextTry?: number) {
    ...
    setIsGenerating(true);
    resetIdleTimer();
    void speak("Karışımı ayarlıyorum… barmen modu 😎");
      ...
      <div className="relative z-10 mx-auto w-full max-w-md px-4 pt-6 pb-[260px] sm:pb-[220px]">
        {screen === "home" && !homeStarted && (
          <>
            ...
            <div className="mt-4 rounded-3xl border border-white/40 bg-white/88 p-5 backdrop-blur-md shadow-[0_10px_28px_rgba(15,23,42,0.10)]">
              <div className="text-[22px] font-black tracking-tight text-[#111827]">Bugünün büyüsü</div>

              <div className="mt-2 text-[15px] font-semibold leading-7 text-slate-700">
          ...
          <div className="grid gap-4 animate-[fadeIn_.25s_ease]">
            ...
            >
              ...
              <div className="mt-2 text-[15px] font-semibold text-white/85">
                Dolap foto → tara → seç → tarif
              </div>
              <div className="mt-3 text-xs font-bold uppercase tracking-wide text-white/60">
                Cin Şef mutfakta
              </div>
            </button>
          ...
          <ChefCin
            ...
            onClick={() => {
              ...
              setTimeout(() => {
                void speak(
                  screen === "home" && !homeStarted
                    ? "Ben buradayım 😎 Günün büyüsüne bas, sonra tarif ya da kokteyl seç."
                    : screen === "home" && homeStarted
                    ? "Tarif mi, kokteyl mi? Seçimini yap 😎"
                    : "Hadi canım… foto ver de biraz şov yapalım 😎",
                  true
                );
              }, 120);
            }}
